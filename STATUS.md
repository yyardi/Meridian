...
| PULSE (WNBA in-game model loop) | **live decisions scored for the first time 2026-09-23** (docs/math/pulse-live-scorecard.md) | 36,754 decisions / 785 markets / **52 games**: model vs venue Brier −0.0012 [−0.0115, +0.0091]; the side it leans to wins 47–53 % in every gap bucket; enters as a taker −0.99c [−3.76, +1.79] per contract (spreads +3.17c, totals −3.20c); filled −3.11c vs withdrawn +1.81c — the unfilled arm flatters. No signal beyond the price at G = 52 |
...
**Also retracted: the fee — and the reason is a venue change, not an original error.** *(Amended 2026-09-21 evening: the first version of this paragraph said the recorder had stored the field "since 2026-09-18" and that every dollar before this line was overstated. Both wrong.)* `market_snapshots.fee_coefficient` has been recorded since 2026-07-31: **0.06** on 63,539,087 rows to 2026-09-17 04:00Z, then **0.0695** on every one of 21,656,635 rows from 04:07Z that day on, across NFL, CFB, WNBA, MLB and table tennis. The venue raised its taker coefficient by 16 % at midnight Eastern on 2026-09-17. V9 (0.06, 2026-08-04) was right on its day, and so was every hand derivation in docs/math written before it. What was wrong was the tree: `DEFAULT_FEE_RATE` and nine other literals were never compared to the field, so for four days (09-17..09-21) every fee was charged 16 % light with every log green — configured, not measured. About half a cent per pair at even prices. Every ladder dollar measured on tapes from 09-17 on was overstated by it (both slates in the table below are recomputed at 0.0695), a sub-cent crossing the old constant reported is not one, and the plausibility cap's boundary moved with it (a 15.1c edge the cap refused is 14.6c and counts). Consolidated in `core/fees.py` — one constant, every site imports it — with `tests/test_fee_is_one_constant.py` sweeping the tree for any fee-shaped 0.06 (its first run found a tenth runner the hand sweep missed) and `scripts/fee_drift.py` comparing the constant to the last day's recorded coefficients in every nightly verdict; 33 tests recomputed. docs/math/fee-coefficient.md. The correction exposed one more defect: the desk rounded a pair's dollars to the cent for display and tested the floor on the raw value, so $24.996 showed as $25.00 and said "not a candidate"; `scan.clears_floor` rounds first and every gate goes through it.
...
**What the ledger says, fee 0.0695, floor $25** (one tape per game: two hand-scheduled recorder windows on 09-19 overlapped thirty minutes and a first pass counted that wave twice; the ledger keeps the fuller tape):

| slate | gate | games | episodes | ≥ $25 | Σ best of those | biggest | median life of those | **spread-only ≥ $25** | **Σ** | **biggest** |
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 2026-09-19 CFB | 2 s | 50 | 5,651 | 26 | $1,451 | $248 | 14.9 s | **5** | **$269** | **$104** |
| 2026-09-19 CFB | none | 50 | 18,556 | 31 | $1,752 | $248 | 24.7 s | 7 | $330 | $104 |
...
## 0ce. Cricket in play: the venue's screen cannot show the state, and the read is registered before the tape exists (2026-09-22)

Japan v India (Asian Games, rain-cut to five overs): the venue's `event_score` read `0-0` through India's innings and `0-32` at 32/4 (5 ov) — no wickets, no overs, no target — while ESPN's summary carried all of it every 15 s and `espn_cricket_events` recorded it. Japan's winner price went 2c → 40c inside 45 minutes and was 10c bid at 9/2 chasing 33. One call at a price is `p` against a coin (docs/math/cricket-preregistration.md §1); the pregame spike a day earlier (3c → 32c → 12c in 45 min, nothing on ESPN) goes to that document's Half B. **Registered today, docs/math/cricket-inplay-dip.md:** primary read is the CALIBRATION of the venue's in-play price by the favourite's drawdown bucket, per-match clustered, rain-affected matches their own row; the decision rule (buy the pre-toss favourite ≥ 25c below its pre-toss quote, hold to settlement, at the row's fee) is written down and runs only if the 25c+ bucket is miscalibrated; ~150 dips for 80 % power; the venue has listed 27 non-county matches in nine days (≈3/day), so that is months. **Infrastructure:** the cricket winner is on the stream recorder (`MATCH_WINNER_TYPES`), the daily scheduler treats `t20icr/t20iwcr/cplcr/odicr` as recorder-only (no detector, no sampler; T20 240 min, ODI 540) and, on the operator's ask that evening, records `county` too for four days from the first ball as its own row (settles 0.5 on a draw, 50–84c spreads; a four-day match does not hold the nightly verdict). Nothing places anything.

## 0cf. The in-game model's live decisions are scored at last, and they carry no signal beyond the price (2026-09-23)

`cfb/run_pulse_live_scorecard.py` (agent build, merged 5274dbe) scores every `pulse_decisions` row against venue settlement, deduped at the (market, instant, action) collision the table has no key for, fee at each row's own coefficient. 36,754 decisions, 785 markets, **52 games** (18 playoff). Model vs venue Brier at the same instants **−0.0012 [−0.0115, +0.0091]**; playoffs +0.0036 [−0.0083, +0.0156]; the side the model leans to wins 47–53 % in every |fair value − mid| bucket. Paper enters as a taker at the recorded touch, held: **−0.99c [−3.76, +1.79]** per contract; filled −3.11c against withdrawn +1.81c, the mid having moved −4.52c into every fill — the unfilled arm flatters, as registered. This is the "signal from game trajectory" the operator asked about on 09-22: it has been running since August, and at G = 52 it does not beat the venue's own price. Detail: docs/math/pulse-live-scorecard.md.

## 0cg. The player-based WNBA pregame model, registered first and read once: the venue's price beats it on 106 games (2026-09-23)

On the operator's ask (a Monte Carlo on the players), an agent wrote docs/math/wnba-player-model-preregistration.md before comparing a price — per-player plus-minus per minute with shrinkage, expected minutes from the last ten visible games with ESPN `Out` designations zeroed, team strength as the minutes-weighted sum, home edge 3, σ walk-forward, 20,000 draws a game, point-in-time by construction (a box score is visible three hours after its own tip) — then `cfb/run_wnba_player_model.py` against the venue's T−1h price with venue settlement. **106 games, all matched and settled: Brier model 0.1951, venue 0.1780, coin 0.2500; Δ model − venue +0.0171 [−0.0012, +0.0354]** (regular 88: +0.0130 [−0.0074, +0.0334]; playoffs 18: +0.0373 [−0.0035, +0.0782], underpowered). The registered gate failed, so the paper line it would have taken (76 bets) is NOT SCORED and nothing is quoted. Who plays tonight is priced by T−1h, as docs/math/availability.md measured before. The runner reruns nightly; only condition 6 (playoffs pass, all fail) is left for it to say, on a 2027 tape.

## 0ch. The $25 floor was about the venue's display, not the operator's bankroll: the ticket gate is now sized to a $20 attempt (2026-09-26)

The operator asked why a $25 limit stood between a $20 test and the ladder. It did: `$25` was edge × the FULL displayed size, which put a crossing with 22 contracts at 5c (worth $1.10 at full size, $1.10 to a $20 test as well) under the floor while a 5,000-contract 0.5c flicker cleared it. The gate is now `core/ladder/intent.clears_ticket_gate`: the thin leg must display at least the contracts a `MERIDIAN_ARB_ATTEMPT_USD` ($20) pair buys, at ≥ 2c net — on the desk (`candidate`), in both executors, and in the planner's launch line, one number. Tickets are sized to the attempt (~22 contracts) and capped by the thin leg and the $25 pair cap. The $25 stays the ledger's statistic; the ledger gains `tkt@$20 / profit / life` — what a $20 attempt could have ticketed. docs/math/ladder-fill-test.md amended.
