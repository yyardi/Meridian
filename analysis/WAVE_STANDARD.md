...
B's cross-market census hit the same failure shape in gap handling. Two
builders, blind to each other, same instrument lesson.

**The rule:** any cross-series comparison on a tick tape must bound
staleness explicitly — a fresh-bucket grid (both series must quote within
the same bucket; the coherence and census instruments use 10s) or an ffill
limit matched to the poll cadence, stated in the artifact. An absent quote
is ABSENT, not the previous value; and per the recorder's own book_tier
principle, distinguish *no resting size* from *we did not look*. A joined
number whose staleness bound is unstated is unreviewable.
...
discrepancy is what surfaced this trap, twice. (Same family as "break a
new check on purpose before trusting it".)
