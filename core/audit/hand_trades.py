...
The numbers in ``docs/math/hand-trade-audit.md`` predate this correction.
...
class Fill:
    ...
    #: ``market.line`` — the total or spread number. Optional with a default so
    #: adding it stayed a widening rather than a break: the audit never needed
    #: it, and the per-fill export (:mod:`core.audit.trade_export`) cannot
    #: label a row without it. None where the venue omits the field — never
    #: recovered from the slug, which encodes the line only by convention.
    line: Decimal | None = None
    #: ``trade.realizedPnl`` — the venue's OWN realized P&L for this trade,
    #: present on 117 of 455 (the closing ones). Carried as a cross-check and
    #: never used to compute: it is the account's book of record, and where it
    #: disagrees with our reconstruction the disagreement is worth seeing
    #: rather than averaging away.
    venue_realized_pnl: Decimal | None = None
    #: ``trade.cost`` and ``trade.costBasis``. Carried because they *explain*
    #: the number above — verified on the live payload,
    #: ``realizedPnl == cost - costBasis`` — and because the venue's basis
    #: convention is undocumented and demonstrably not ours. See V22.
    venue_cost: Decimal | None = None
    venue_cost_basis: Decimal | None = None
...
def parse_activity(raw: dict) -> tuple[list[Fill], Resolution | None, bool]:
    ...
    return [Fill(
        ...
        or ZERO,
        line=_dec(market.get("line")),
        venue_realized_pnl=_dec((trade.get("realizedPnl") or {}).get("value")),
        venue_cost=_dec((trade.get("cost") or {}).get("value")),
        venue_cost_basis=_dec((trade.get("costBasis") or {}).get("value")),
    )], None, True
