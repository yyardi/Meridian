...
| B15 | **`exp_devigged_clv.analyse()` computes every expected-ROI and CLV number and has NO TEST — and a sign error can hide inside it** | $0 so far; unquantified — every CLV figure this module has produced rests on an untested branch | `core/backtest/exp_devigged_clv.py:133`, `p_entry_novig = devigged[0] if b.side == "over" else devigged[1]`. **Swap that index and every CLV number in the project flips sign.** The side selection, the filter chain at 128-140 and the `won is not None` branch are all unexercised. The two tests named for this computation — `tests/test_devigged_clv.py:100,108` — assert `0.55 / 0.50 - 1 == approx(0.10)`, i.e. **facts about floating-point division**, under a section header reading *"Buchdahl's rule, end to end"*. The file imports `closing_probability`, `devig_two_sided`, `interval` and **nothing that computes ROI**; delete `analyse()` entirely and both tests stay green | **A deliberate achievable-image pass over the suite** (2026-09-06): for each test, does it assert while reaching no product symbol, no local helper reaching one, no file IO and no fixture? 88 files → 13 candidates → **2 confirmed unable to fail**, these two. Note the first cut of that scan returned **272** candidates and was discarded: **an instrument that flags a third of the suite is not an instrument.** THE FIX, SPECIFIED: import `analyse`, build a `BacktestResult` with **two known bets, one `over` and one `under`**, and assert the returned `expected` and `clv_prob` against hand values. **The over/under PAIR is the whole point — a single-sided fixture reproduces the current blind spot instead of closing it, so do not "simplify" it to one bet.** Same shape as B-series absent-variable (the field deciding the sign missing from every fixture) and the 2026-09-05 monotone constraint that was *satisfied* and *unused* |
...
| C15 | Two same-day claims about the venue-freeze alarm's constants (2026-09-06). Mine, **pre-registered before the prod run**: "the 30-minute healthy price-move share will land near **49%**". The manager's, on seeing the result: "`pct_below_floor = 0.000` **validates** `MIN_MARKETS = 12`" | **Both wrong, in different ways, and the second is the dangerous one because it looked like success.** (a) I fitted **one common rate** to an hourly figure and projected down. `P(move) = 1 − exp(−λT)` is **concave in λ**, so heterogeneous rates *flatten* the dependence on window length: the homogeneous limit gives 49.4%, the fully-heterogeneous limit **74.4% at any T**. The answer had to lie between two endpoints computable in one line, and I quoted the lower one as a point estimate — having named the common-rate assumption in the docstring and never computed its direction. **Naming a caveat is not bounding it; a caveat that does not move the number is decoration.** (b) The clean null was measured **entirely outside the regime the parameter governs**: the thinnest qualifying bucket held **42 markets**, while `MIN_MARKETS` guards the **12–24** band, where the sample held nothing. At 42 markets a false page needed P ~ 1e-24 — the zero confirmed a case that was already impossible and never entered the hard one | Measured (33 buckets, 30-min, freeze excluded, CFB): median **84.2%**, p05 73.2%, p01 69.7%, worst **69.0%**. **`FLOOR_PCT = 5.0` is VALIDATED** — a ~14× margin to the worst healthy bucket, zero healthy buckets below it; the point-mass framing holds for the reason it was chosen. **`MIN_MARKETS = 12` is UNVALIDATED**: 8, 12 and 14 are indistinguishable on this sample, and the extrapolation runs the **wrong way** — thin slates are thin *because* fewer games are live (later, more decided, genuinely quieter), so the per-market rate at the floor is plausibly **lower** than 84.2% and the false-positive risk there **worse** than anything derived from dense buckets. Fix: `scripts/healthy_distribution.sql` now prints **`near_floor_buckets`** — if it returns 0, the run said nothing about the floor however clean `pct_below_floor` looks, and the header names this run as the instance both of us misread. **A number that reassures without testing anything is worse than a missing one, because a missing one prompts a question.** Thin-slate search over wider history is the deciding measurement and is queued |
| C16 | The thin-slate search's first result: **"the near-floor 12-24 band shows a 2.010% sub-floor rate"**, read as evidence that `MIN_MARKETS = 12` was unsafe (2026-09-06). More broadly, every query in this project that identifies live markets by `is_live = true` alone | **`is_live` IS NEVER CLEARED — 6,591 markets carry a frozen last row.** The flag is a faithful record of an **instant**, not a **state**. So a market whose stream died reads as *quoted, not moving* — **indistinguishable from a frozen venue by the exact statistic the freeze alarm uses.** The whole 2.010% was stale-flag artefact. **The tell was `worst = 0.0` in EVERY WNBA band including the dense one: an exact zero in a population that cannot produce exact zeros is a data-defect signature, not an observation** — a genuine quiet slate does not produce exact zeros at 42+ markets, a set of dead streams does. **★ And the same defect was in the ALARM ITSELF**, which filtered raw `is_live` with no freshness guard, so it would have suffered both failure modes in production: dead streams inflating the denominator until a thin slate looked readable, *and* counting as non-movers to drag the statistic toward zero. The contaminated measurement was a fair forecast of the shipped instrument's real behaviour | Fix: a **600s stream-running guard** — a row counts only if its market produced another within 600s. Guarded, the near-floor band goes **2.010% → 0.000%** and `worst` goes **0.0% → 44.4%**; 17 buckets (199 → 182) were composed *entirely* of dead streams. **Both constants are now validated on the regime they govern**: `MIN_MARKETS = 12` with **182 buckets sitting in the 12-24 band — the most populated band on the tape, so the blind spot was never theoretical — and zero sub-floor buckets in it**; `FLOOR_PCT = 5.0` with an **8.9x** margin at the worst judged bucket (down from the 13.8x quoted off the contaminated dense-only sample: smaller, real, still comfortable). The only band still showing sub-floor buckets is the one the alarm **refuses** to judge (1-11 markets → INSUFFICIENT), which is the three-valued design working. **The guard is load-bearing, not an optimisation: without it the alarm pages ~22x/month on healthy data.** Critically it does NOT suppress the true positive — during the 09-05 freeze the recorder kept writing at 8.4 batches/min, so gaps were ~7s and every frozen row clears a 600s guard. Asserted verbatim against the module's own SQL string by `scripts/test_alarm_v5_guard.sh` |
