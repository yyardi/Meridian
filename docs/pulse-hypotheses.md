...
| 20 | signal | **Q4 endgame-state FV calibration.** Does endgame state — possession, bonus, timeouts — added to the pregame-anchored win curve improve Q4 win-probability calibration? Serves the EV stop (exit quality), not entries | research agent; the operator's Q4 focus and the F8 bound design (disclosure in the registration) | **AMENDED 2026-08-25, before any real data.** Primary is now the possession coefficient in the deployable arm's full-sample offset fit, game-clustered 95% Wald CI; PASS = excludes zero. The original paired-Brier gate was **withdrawn as unwinnable** — it could not detect an injected k=1.2 effect at 60 synthetic games. Arms, cohort, floors, LOGO and disclosure unchanged; held-out Brier survives as a descriptive calibration line, never gating | **REGISTERED 2026-08-25, NOTHING REAL COMPUTED.** Amended after the harness failed its own mutation test on synthetic data — the instrument failed commissioning, no real data touched. A STATE test on the surviving side of F8; the possession leg is flagged at risk from that bound. Carries a **named-casualty clause** (ceiling passes where deployable does not → "the possession leg is an F8 casualty", verbatim) and an **informative-not-calibrating clause** (coefficient CI excludes zero but the descriptive Brier line shows no improvement → "informative, not yet demonstrated to improve calibration", not a clean PASS). Run by the research agent. [math/q4-endgame-state.md](math/q4-endgame-state.md) |
