...
| 16 | signal | **Trailing-team live ML underpricing.** In tight games the market prices the trailing team's live win probability below the historical base rate for that game state | *"i took IND to win at 30% when they were down just 5 in a tight game... cashed at 45"* (2026-08-06, real money, +50%) | pre-register in the module docstring before computing; needs the historical curve (787 games) + ≥10 games of live ML ticks | **not built — prompt written 2026-08-07.** The only hypothesis so far with live-money wins attached to its origin. Shares the P(win \| margin, time) curve with [[#15]] |
| 15 | input | **Team-specific lead survival.** Some teams hold leads worse than the league curve; a live model's P(win \| lead, time) should know which | *"dallas isnt great at holding onto leads theyve blown so many"* (2026-08-06) | — | not built. **Measurable today from the 787 historical box scores**: fit league-wide lead-survival curves (lead size × time remaining), then per-team deviation. Two cautions, both load-bearing: a per-team trait at WNBA sample sizes is mostly noise (13 teams, few blown-lead events each), so the team term must be shrunk toward the league curve; and the market watches the same games — the tradable question is whether the *price* under-reacts to the trait, not whether the trait exists |
