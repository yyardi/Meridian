...
| 20 | signal | **Q4 endgame-state FV calibration.** Does endgame state — possession, bonus, timeouts — added to the pregame-anchored win curve improve Q4 win-probability calibration? Serves the EV stop (exit quality), not entries | research agent; the operator's Q4 focus and the F8 bound design (disclosure in the registration) | paired Brier difference (base − state-deployable) on held-out ticks, game-clustered 95% CI (C4); dual floor ≥15 settled signal-covered games AND ≥1,000 held-out cohort ticks; PASS = CI excludes zero in the state arm's favour. Ceiling arm (wallclock) is diagnostic and never gate-eligible | **REGISTERED 2026-08-25, NOTHING COMPUTED.** A STATE test on the surviving side of F8; the possession leg is flagged at risk from that bound before any number exists. Carries a **named-casualty clause**: if the ceiling arm passes where the deployable arm does not, the verdict must read "the possession leg is an F8 casualty" in exactly those words. Run by the research agent. [math/q4-endgame-state.md](math/q4-endgame-state.md) |
