...
| 6 | signal | **Tail volatility at the edges.** Deep rungs move most at the start and end of a game. | *"the tail odds move a ton at the start and end"* | both edges > mid, CI excluding zero, 10 games | **NO DATA — and pointing away, 2026-08-07.** [`core/pulse/tail_volatility.py`](../core/pulse/tail_volatility.py). Open edge has only **8 games** so the gate cannot close — but its interval is **−0.731¢ [−0.982, −0.480]**, entirely below zero: tails are *quieter* at the open. Close edge passes (+0.555¢) while the **body** gains +2.135¢, ~4× more, so that half is a whole-board phase effect, not a tail effect — the pre-registered control firing. **Deep-tier rungs are unmeasurable at any sample size**: 30s cadence vs the near tier's 0.20s (C1). [math/tail-volatility.md](math/tail-volatility.md) |
