...
Then over the next 2 minutes:
  ...
  taker_pnl   trade AGAINST the slow side at t (ask-led DOWN move -> sell at the
              ...
              mark at the mid at t+2, minus the fee c*p*(1-p) at the trade price, c the
              coefficient recorded on minute t's own row (the venue raised it on 2026-09-17).
...
def observe(prev, now, later):
    """One minute's move, priced at what a TAKER against the slow side would get.

    prev / now / later are (bid, ask, fc) at t-1, t, t+2. fc is market_snapshots.fee_coefficient
    on that minute's row; the trade at t is charged at NOW's (None raises, core.fees.recorded_fee).
    Returns (kind, other_side_follow, taker_pnl_net, taker_pnl_gross, |dm|), None below 1c."""
    b0, a0, _ = prev; b1, a1, fc = now; b2, a2, _ = later
    dm = ((b1+a1) - (b0+a0)) / 2
    if abs(dm) < 0.01: return None
    db, da = b1 - b0, a1 - a0
    ...
    return kind, follow, pnl - recorded_fee(px, fc), pnl, abs(dm)
...
with eng.connect() as c:
    ...
    snaps = [dict(r._mapping) for r in c.execute(text("""
        SELECT game_id, market_slug, captured_at, best_bid::float AS bid, best_ask::float AS ask,
               fee_coefficient::float AS fc FROM market_snapshots
...
for g in games:
    ...
    minute = {}
    for s in rs: minute[int((s["captured_at"] - ko).total_seconds() // 60)] = (s["bid"], s["ask"], s["fc"])
    ks = sorted(minute)
    for i in range(1, len(ks) - 2):
        if ks[i] - ks[i-1] != 1 or ks[i] + 2 not in minute: continue
        o = observe(minute[ks[i-1]], minute[ks[i]], minute[ks[i]+2])
        if o is None: continue
        kind, follow, net, gross, adm = o
        obs[kind].append((100*follow, 100*net, 100*gross, g["venue_game_id"], adm))
...
print(f"LEAGUE={LEAGUE}  games {len(games)}  (EXPLORATORY on CFB; registered H1b for NFL)"
      f"  fee coefficients on tape: {sorted({s['fc'] for s in snaps}, key=lambda x: (x is None, x))}")
print(f"\n=== who moved first, and did the other side follow within 2 min? ===")
net_hdr = "NET of fee at the row's coefficient"
print(f"  {'kind':<9}{'n':>6}{'G':>4}   {'other side follows':>19}{'95% CI':>18}   {'taker vs slow side, GROSS':>26}   {net_hdr:>36}")
