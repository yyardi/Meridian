#!/usr/bin/env bash
# launch_stream_exec.sh <winner-slug> <minutes> [floor] [fresh_s]
...
docker run -d --rm --name "sexec-${G:0:20}" --network meridian_default --env-file /opt/meridian/.env \
  ...
  "$API" sh -c "python cfb/run_stream_executor.py --prefix $S --minutes $M --floor-usd $FLOOR --fresh-s $FRESH > /out/stream_exec_${S}.txt 2>&1"
