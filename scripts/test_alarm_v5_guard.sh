...
PY="${MERIDIAN_PY:-$REPO/.venv/bin/python}"
[ -x "$PY" ] || PY="$(command -v python3)"   # worktrees have no .venv
export SCRIPTS
# Resolved from THIS FILE, not hardcoded: the absolute path it
# carried pointed at the main checkout, so running the guard from
# a worktree silently tested a DIFFERENT alarm_v5 than the one
# beside it — and put a home directory in a public repo.
C=guard_test
...
"$PY" - <<'PY'
import os, sys; sys.path.insert(0, os.environ['SCRIPTS'])
